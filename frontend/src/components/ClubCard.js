...
const ClubCard = ({ club }) => {
  const broadcastingCodes = {
    ...
    'Derby County': { code: 'DER', color: 'warning' },
    ...
    'Preston North End': { code: 'PRE', color: 'secondary' },
    ...
    'Swansea City': { code: 'SWA', color: 'secondary' },
  ...
  return (
    <Link to={`/club/${encodeURIComponent(club.club_name)}`} className="text-decoration-none">
      <div className="py-3 px-3 dark-list-item">
